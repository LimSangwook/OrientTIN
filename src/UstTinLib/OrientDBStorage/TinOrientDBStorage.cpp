...
String CTinOrientDBStorage::_GetProperty(String json, String propertyName)
{
	...
	if (propertyName.compare("out") == 0 || propertyName.compare("in") == 0 ) {
		int startpos = retStr.find('#');
		int endpos = retStr.find('{');
		retStr = retStr.substr(startpos, endpos-startpos);
	}
	return retStr;
}
...
VertexPtr CTinOrientDBStorage::GetVertex(RID vertexRID)
{
	...
	std::cout << "GetVertex(vertexRID) " << vertexRID << " : " << strV << "\n";
	VertexPtr vPtr = _GetStringToVertex(strV);
